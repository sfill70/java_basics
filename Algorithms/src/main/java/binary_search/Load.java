...
public class Load {
    ...
    static String query = "Предупреждение: книга содержит элементы живого разговорного русского языка (менее 0,016 % текста). Могут быть задеты сексуальные, политические и религиозные чувства читательницы, а также ее шизофренические и параноидальные комплексы. Автор не ставит такой цели и не несет ответственности за эффекты, возникающие в уме «Б» во время выработки агрегата «М5».\n" +
            "     \n" +
            "     \n" +
            "РИДМИДОК\n" +
            "     \n" +
    ...
    public static void main(String[] args) {
        BinarySearch binarySearch = new BinarySearch(new ArrayList<String>(Arrays.asList(query.replaceAll("[\\s]+", " ").split(("[\\s]")))));
//        System.out.println(new ArrayList<String>(Arrays.asList(query.replaceAll("[\\s]+", " ").split(("[\\s]")))));
        System.out.println(binarySearch.search("магические12"));
        System.out.println(binarySearch.getList().get(binarySearch.search("магические")));
        System.out.println(binarySearch.getList().size());
    }
